# -*- coding: utf-8 -*-
""" StrategicEvaluator class to wrap and run the simulations.
...
try:
    ...
    from .StrategicResult import StrategicResult
...
class StrategicEvaluator(object):
    """ Evaluator class to run the strategic simulations."""
    ...
    def __init__(self, configuration,
        ...
        # SlyJabiru
        self.armChosenNb = dict()
        self.agentChosenNb = dict()
        self.rewardsPerArms = dict()
        self.rewardsPerAgents = dict()
        self.instantRegret = dict()
        # self.choices = dict() # time -> arm
        # self.agentChoices = dict() # time -> agent
        ...
        for envId in range(len(self.envs)):
            ...
            # SlyJabiru
            self.armChosenNb[envId] = np.zeros((self.nbPolicies, self.envs[envId].nbArms, self.repetitions), dtype=np.int32)
            self.agentChosenNb[envId] = np.zeros((self.nbPolicies, len(self.envs[envId].nbArmsPerAgents), self.repetitions), dtype=np.int32)
            self.rewardsPerArms[envId] = np.zeros((self.nbPolicies, self.envs[envId].nbArms, self.repetitions))
            self.rewardsPerAgents[envId] = np.zeros((self.nbPolicies, len(self.envs[envId].nbArmsPerAgents), self.repetitions))
            self.instantRegret[envId] = np.zeros((self.nbPolicies, self.horizon, self.repetitions))

            # self.choices[envId] = np.zeros((self.nbPolicies, self.horizon, self.repetitions), dtype=np.int32)
            # self.agentChoices[envId] = np.zeros((self.nbPolicies, self.horizon, self.repetitions), dtype=np.int32)
            ...
            # TODO! regret 새로 만들어버려
            # instantRegret = np.zeros((self.nbPolicies, self.horizon)) 여기다가, bestArmMean - PulledArmMean 을 계속 여기다가 담아
            # 그 다음에, cumsum 하면 cumulated regret 이야.
            # 미쳤다 미쳤어
    ...
    def startOneEnv(self, envId, env):
        ...
        def store(r, policyId, repeatId):
            # TODO: Modify here, to cover agent pulls and agent rewards
            # rewards -> arm reward, pulls -> arm pulled
            # TODO: make agentReward, agentPulls
            # 여기도, agent offset 구해서, agent pulls reward 올려주면 될 듯
            ...
            # SlyJabiru
            self.armChosenNb[envId][policyId, :, repeatId] += r.pulls
            self.agentChosenNb[envId][policyId, :, repeatId] += r.agentChosenNb
            self.rewardsPerArms[envId][policyId, :, repeatId] += r.rewardsPerArms
            self.rewardsPerAgents[envId][policyId, :, repeatId] += r.rewardsPerAgents
            self.instantRegret[envId][policyId, :, repeatId] += r.instantRegrets
    ...
    def saveondisk(self, filepath="saveondisk_Evaluator.hdf5"):
        ...
        for envId in range(len(self.envs)):
            ...
            # SlyJabiru
            for name_of_attr in ["isChangingAtEachRepetition", "isMarkovian", "_sparsity", "means", "nbArms",
                                 "nbArmsPerAgents", "ArmTypeString",
                                 "maxArm", "minArm"]:
            ...
            for methodName in ["getRunningTimes", "getMemoryConsumption", "getNumberOfCPDetections", "getBestArmPulls",
                               "getMeanArmChosenNb", "getMeanAgentChosenNb", "getMeanRewardsPerArms", "getMeanRewardsPerAgents",
                               "getStdArmChosenNb", "getStdAgentChosenNb", "getStdRewardsPerArms", "getStdRewardsPerAgents"
                            #    "getChoices", "getAgentChoices",
                            #    "getPulls",
                               "getRewards", "getCumulatedRegret", "getLastRegrets", "getAverageRewards"]:
    ...
    def getMeanArmChosenNb(self, policyId, envId=0):
        """Extract mean armChosenNb."""
        return np.mean(self.armChosenNb[envId][policyId], axis=1)

    def getMeanAgentChosenNb(self, policyId, envId=0):
        """Extract mean agentChosenNb."""
        return np.mean(self.agentChosenNb[envId][policyId], axis=1)

    def getMeanRewardsPerArms(self, policyId, envId=0):
        """Extract mean rewardsPerArms."""
        return np.mean(self.rewardsPerArms[envId][policyId], axis=1)

    def getMeanRewardsPerAgents(self, policyId, envId=0):
        """Extract mean rewardsPerAgents."""
        return np.mean(self.rewardsPerAgents[envId][policyId], axis=1)

    def getCumulatedRegret(self, policyId, envId=0, moreAccurate=None):
        cumulatedRegret = np.cumsum(self.instantRegret[envId][policyId], axis=0)
        return np.mean(cumulatedRegret, axis=1)


    def getStdArmChosenNb(self, policyId, envId=0):
        """Extract std armChosenNb."""
        return np.std(self.armChosenNb[envId][policyId], axis=1)

    def getStdAgentChosenNb(self, policyId, envId=0):
        """Extract std agentChosenNb."""
        return np.std(self.agentChosenNb[envId][policyId], axis=1)

    def getStdRewardsPerArms(self, policyId, envId=0):
        """Extract std rewardsPerArms."""
        return np.std(self.rewardsPerArms[envId][policyId], axis=1)

    def getStdRewardsPerAgents(self, policyId, envId=0):
        """Extract std rewardsPerAgents."""
        return np.std(self.rewardsPerAgents[envId][policyId], axis=1)

    def getSTDRegret(self, policyId, envId=0, meanReward=False):
        cumulatedRegret = np.cumsum(self.instantRegret[envId][policyId], axis=0)
        return np.std(cumulatedRegret, axis=1)
    ...
    def getLastRegrets_LessAccurate(self, policyId, envId=0):
        ...
        cumulatedRegret = np.cumsum(instantRegret, axis=2)
...
def delayed_play(env, policy, horizon,
    ...
    bestArmMean = np.max(means)

    # Start game
    policy.startGame()
    # SlyJabiru
    result = StrategicResult(env.nbArms, horizon, env.nbArmsPerAgents, means, bestArmMean,
                             indexes_bestarm=indexes_bestarm)  # One Result object, for every policy
    ...
    for t in prettyRange:
        ...
        # choice: arm offset, reward: reward from arm
...
def StrategicEvaluatorFromDisk(filepath='/tmp/saveondiskEvaluator.hdf5'):
    ...
    with open(filepath, 'r') as hdf:
        ...
        evaluator = StrategicEvaluator(configuration)
