...
class H_UCB(StrategicIndexPolicy):
    def computeAgentIndex(self, agent):
        if self.agentPulls[agent] < 1:
            return float('+inf')
        else:
            return (self.agentRewards[agent] / self.agentPulls[agent]) + sqrt((2 * log(self.t)) / self.agentPulls[agent])
    ...
    def computeAllIndex(self):
        ...
        agentIndices = (self.agentRewards / self.agentPulls) + np.sqrt((2 * np.log(self.t)) / self.agentPulls)
